...
namespace argparse
{
  ...
  std::vector<std::string> inline split(const std::string& str)
  {
    ...
    while (std::getline(ss, key, ','))
    {
      ...
      // comparing to raw string, e.g. value == "test" will fail
      // when the last character is '\0'. Therefore we can
      // remove it
  ...
  template <typename T>
  std::string
    to_lower(const T& str_)
  { // both std::string and std::basic_string_view<char>
    // (for magic_enum) are using to_lower
  ...
  template <>
  inline char get(const std::string& v)
  {
    return v.empty() ? throw std::invalid_argument("empty string")
      : v.size() > 1 ? v.substr(0, 2) == "0x"
      ? (char)std::stoul(v, nullptr, 16)
      : (char)std::stoi(v)
      : v[0];
  }
  template <>
  ...
  template <>
  inline unsigned char get(const std::string& v)
  {
    return get<char>(v);
  }
  template <>
  inline unsigned int get(const std::string& v)
  {
    return std::stoul(v);
  }
  template <>
  inline unsigned short get(const std::string& v)
  {
    return std::stoul(v);
  }
  template <>
  inline unsigned long get(const std::string& v)
  {
    return std::stoul(v);
  }
  ...
  struct ConvertType : public ConvertBase
  {
    ...
    {
      if (this->get_type_id() ==
        default_value->get_type_id()) // When the types do not match exactly.
  ...
  struct Entry
  {
    ...
    // Allow both string inputs and direct-type inputs. Where a string-input will
    // be converted like it would when using the commandline, and the direct
    // approach is to simply use the value provided.
    template <typename T>
    ...
    operator T& ()
    {
      // Automatically set the default to nullptr for pointer types and empty for
      // optional types
    ...
    template <typename T>
    operator T() {} // When you get here  because you received an error, make sure
    // all parameters of argparse are references (e.g. with `&`)
  ...
  private:
    ...
    void _apply_default()
    {
      ...
      else if (default_str_
        .has_value())
      { // in cases where a string is provided to the
    ...
    [[nodiscard]] std::string info() const
    {
      ...
      const std::string allowed_value =
        ...
        ? "allowed: <" +
        allowed_entries.substr(0, allowed_entries.size() - 2) + ">, "
        : "";
  ...
  struct SubcommandEntry
  {
    ...
    explicit SubcommandEntry(std::string subcommand_name)
      : subcommand_name(std::move(subcommand_name)) {}

    template <typename T>
    operator T& ()
    {
      static_assert(std::is_base_of_v<Args, T>,
        "Subcommand type must be a derivative of argparse::Args");
  ...
  public:
    ...
    /* Add a positional argument, the order in which it is defined equals the
     * order in which they are being read. help : Description of the variable
     *
     * Returns a reference to the Entry, which will collapse into the requested
     * type in `Entry::operator T()`
     */
    ...
    /* Add a *named* positional argument, the order in which it is defined equals
     * the order in which they are being read. key : The name of the argument,
     * otherwise arg_<position> will be used help : Description of the variable
     *
     * Returns a reference to the Entry, which will collapse into the requested
     * type in `Entry::operator T()`
     */
    Entry& arg(const std::string& key, const std::string& help)
    {
      std::shared_ptr<Entry> entry =
        std::make_shared<Entry>(Entry::ARG, key, help);
      // Increasing _arg_idx, so that arg2 will be arg_2, irregardless of whether
      // it is preceded by other positional arguments
      _arg_idx++;
     ...
     * key : A comma-separated string, e.g. "k,key", which denotes the short (-k)
     * and long(--key) keys_ help : Description of the variable implicit_value :
     * Implicit values are used when no value is provided.
     *
     * Returns a reference to the Entry, which will collapse into the requested
     * type in `Entry::operator T()`
     */
    Entry&
      kwarg(const std::string& key, const std::string& help,
     ...
     * key : A comma-separated string, e.g. "k,key", which denotes the short (-k)
     * and long(--key) keys_ help : Description of the variable
     *
    ...
    /* Add a a subcommand
     * command : name of the subcommand, e.g. 'commit', if you wish to implement a
     * function like 'git commit'
     *
     * Returns a reference to the Entry, which will collapse into the requested
     * type in `Entry::operator T()` Expected type *Must* be an std::shared_ptr of
     * derivative of the argparse::Args class
     */
    SubcommandEntry& subcommand(const std::string& command)
    {
      std::shared_ptr<SubcommandEntry> entry =
        std::make_shared<SubcommandEntry>(command);
      subcommand_entries[command] = entry;
      return *entry;
    }

    virtual void welcome() {} // Allow to overwrite the `welcome` function to add
    // a welcome-message to the help output
    virtual void help()
    {
      welcome();
      ...
      for (const auto& entry : arg_entries)
      {
        cout << setw(17) << entry->keys_[0] << " : " << entry->help
          << entry->info() << endl;
      }
     ...
     * constructor Upon error, it will print the error and exit immediately if
     * validation_action is ValidationAction::EXIT_ON_ERROR
     */
    void parse(int argc, const char* const* argv, const bool& raise_on_error)
    {
      ...
      argc =
        parse_subcommands(); // argc_ is the number of arguments that should be
      // parsed after the subcommand has finished parsing
      ...
      auto is_value = [&](const size_t& i) -> bool
        {
          return params.size() > i &&
            (params[i][0] != '-' ||
              (params[i].size() > 1 &&
                std::isdigit(
                  params[i][1]))); // check for number to not accidentally mark
          // negative numbers as non-parameter
        };
      auto parse_param =
        [&](size_t& i, const std::string& key, const bool is_short,
          const std::optional<std::string>& equal_value = std::nullopt)
        {
          ...
          if (itt != kwarg_entries.end())
          {
            ...
            { // short values are not allowed to look
              // ahead for the next parameter
              if (is_value(i + 1))
              {
              ...
              {
                entry->_convert(
                  ""); // for multiargument parameters, return an empty vector
                // when not passing any more values
              }
              else
              {
      ...
      for (size_t i = 0; i < params.size(); i++)
      {
        if (!is_value(i))
        {
          ...
          { // short -
            const size_t j_end =
              std::min(params[i].size(), params[i].find('=')) - 1;
      ...
      size_t arg_i = 0;
      for (;
        arg_i < arg_entries.size() && !arg_entries[arg_i]->_is_multi_argument;
        arg_i++)
      { // iterate over positional arguments until a multi-argument
        // is found
      ...
      for (size_t j_end = arg_entries.size() - arg_i; arg_j <= j_end;
        arg_j++)
      { // iterate from back to front, to ensure non-multi-arguments
        // in the front and back are given preference
        ...
        if (flat_idx < arguments_flat.size() && flat_idx >= arg_i)
        {
          ...
          {
            std::stringstream s; // Combine multiple arguments into 1
            // comma-separated string for parsing
